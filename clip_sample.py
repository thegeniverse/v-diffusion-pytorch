...
def resize_and_center_crop(image, size):
    ...
    image = image.resize((int(fac * image.size[0]), int(fac * image.size[1])),
                         Image.LANCZOS)
...
def make_cond_model_fn(model, cond_fn):
    def cond_model_fn(x, t, **extra_args):
        with torch.enable_grad():
            ...
            pred = x * alphas[:, None, None, None] - v * sigmas[:, None, None,
                                                                None]
            cond_grad = cond_fn(x, t, pred, **extra_args).detach()
            v = v.detach() - cond_grad * (sigmas[:, None, None, None] /
...
def main():
    ...
    p.add_argument('--clip-guidance-scale',
                   '-cs',
                   type=float,
                   default=500.,
    ...
    p.add_argument('--starting-timestep',
                   '-st',
                   type=float,
                   default=0.9,
    ...
    clip_model_name = model.clip_model if hasattr(model,
                                                  'clip_model') else 'ViT-B/16'
    ...
    if args.init:
        ...
        init = utils.from_pil_image(init).to(device)[None].repeat(
            [args.n, 1, 1, 1])
    ...
    for prompt in args.images:
        ...
        embeds = F.normalize(clip_model.encode_image(normalize(batch)).float(),
                             dim=-1)
    ...
    if not target_embeds:
        raise RuntimeError(
            'At least one text or image prompt must be specified.')
    ...
    clip_embed = F.normalize(target_embeds.mul(weights[:, None]).sum(
        0, keepdim=True),
                             dim=-1)
    ...
    def run_all(
        ...
        diffusion_linspace = torch.linspace(
            1,
            0,
        ...
        if model.min_t == 0:
            diffusion_steps = utils.get_spliced_ddpm_cosine_schedule(
                diffusion_linspace, )

        else:
            diffusion_steps = utils.get_ddpm_schedule(diffusion_linspace, )

        if args.init:
            diffusion_steps = diffusion_steps[
                diffusion_steps < args.starting_timestep]
            alpha, sigma = utils.t_to_alpha_sigma(diffusion_steps[0])
            init_latents = init * alpha + init_latents * sigma

        for i in trange(0, num_samples, batch_size):
            cur_batch_size = min(num_samples - i, batch_size)
            outs = run(
                x=init_latents[i:i + cur_batch_size],
                steps=diffusion_steps,
                clip_embed=clip_embed[i:i + cur_batch_size],
            )
